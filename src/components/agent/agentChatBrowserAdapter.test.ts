...
import {
  ...
  createAgentChatBrowserAdapter,
  type AgentChatClipboardDocument,
  type AgentChatScheduler,
} from "@/components/agent/agentChatBrowserAdapter";

describe("agentChatBrowserAdapter", () => {
  test("writes through the clipboard API when available", async () => {
    const writes: string[] = [];
    const adapter = createAgentChatBrowserAdapter({
      clipboard: {
  ...
  test("falls back to a hidden textarea when clipboard access fails", async () => {
    ...
    const adapter = createAgentChatBrowserAdapter({
      ...
      clipboardDocument: clipboardDocumentFixture({
        created,
        selected,
        removed,
        copyResult: true,
  ...
  test("schedules reset work with the configured delay and supports cancellation", () => {
    ...
    const scheduler: AgentChatScheduler = {
    ...
    const adapter = createAgentChatBrowserAdapter({ scheduler });
...
function inertScheduler(): AgentChatScheduler {
  return {
    setTimeout() {
      return 0;
    },
...
function clipboardDocumentFixture(options: {
  readonly created: string[];
  readonly selected: string[];
  readonly removed: string[];
  readonly copyResult: boolean;
}): AgentChatClipboardDocument {
  return {
