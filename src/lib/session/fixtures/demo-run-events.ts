...
export function demoEvent(event: DemoEventPayload): RuntimeEvent {
  demoEventSeq += 1;
  return {
    ...event,
    eventId: `demo-evt-${demoEventSeq}`,
    taskId: DEMO_TASK_ID,
    timestamp: demoEventSeq * 1_000,
  } as RuntimeEvent;
}

/** Ordered runtime events replayed in demo mode. Projection shape is verified in tests via `projectSession(demoRunEvents)`. */
export const demoRunEvents: RuntimeEvent[] = [
  demoEvent({
