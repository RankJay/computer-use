...
- On macOS, UI automation needs Accessibility (and often Input Monitoring) under System Settings → Privacy & Security. On accessibility_permission_denied: tell the user to enable Actuate there (then restart Actuate if the toggle was just flipped). window_focus can still activate an app without Accessibility; snapshot/click/type cannot.`
