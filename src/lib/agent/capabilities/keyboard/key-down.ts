...
export const keyDownInputSchema = z.object({
  key: z.string().min(1).describe("Key to hold (e.g. shift, ctrl, alt). Pair with key_up."),
});
...
export const keyDownCapability = defineCapability({
  name: "key_down",
  description:
    "Press and hold a key without releasing. Use for Shift multi-select, Ctrl add-select, or modifier + drag.",
  risk: "high",
